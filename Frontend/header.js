...
fetch("header.html")
  ...
  .then(html => {
    ...
    // ── Auto-redirect profile link based on login status ──
    const profileLink = document.querySelector('.sb-dropdown a[href="account.html"]');
    if (profileLink) {
        const user = JSON.parse(localStorage.getItem('user') || 'null');
        if (user && user.role) {
            if (user.role === 'Student')   profileLink.href = 'student-profile.html';
            if (user.role === 'Professor') profileLink.href = 'professor-profile.html';
            if (user.role === 'Admin')     profileLink.href = 'admin-profile.html';
        }
    }
...
// ===================================================================
// 🤖 AI BOT - Load on all pages
// ===================================================================
(function loadAIBot() {
    const script = document.createElement('script');
    script.src = 'ai-bot.js';
