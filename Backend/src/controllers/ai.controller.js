...
function buildSemesterLinks(branch) {
    const branchName = getBranchDisplayName(branch);
    const branchPrefix = branch.toLowerCase();
    const links = [];

    for (let semester = 1; semester <= 8; semester++) {
        links.push(`<a href="/${branchPrefix}${semester}.html" target="_blank">${branchName} Semester ${semester}</a>`);
    }

    return links;
}

function isBranchNavigationQuery(message) {
    const msg = message.toLowerCase();
    const hasBranch = /\b(me|mechanical|ee|electrical|ce|civil)\b/.test(msg);
    const asksForNavigation = /\b(subject|subjects|semester|semesters|branch|notes|resources|materials)\b/.test(msg);
    const hasSpecificSemester = /sem(?:ester)?\s*[-]?\s*(\d)|(\d)(?:st|nd|rd|th)\s+sem/i.test(msg);

    return hasBranch && asksForNavigation && !hasSpecificSemester;
}
...
async function chat(req, res) {
    try {
        ...
        if (isPersonalQuery) {
            ...
            if (msg.includes("email")) reply = `Your email is: ${user.email}`;
            else if (msg.includes("name")) reply = `Your name is: ${user.username}`;
            else if (msg.includes("branch")) reply = `Your branch is: ${user.branch}`;
            else reply = `Name: ${user.username}\nEmail: ${user.email}\nBranch: ${user.branch}`;
        ...
        29. If user asks to change talking language, just change language and keep all other guidelines same. Always maintain the same intent and information quality regardless of language. Vary phrasing in that language as well.
        30. Try making humors sometimes when the user is asking for something in a funny way, but keep it light and student-friendly. Don't force humor, only when it fits the context naturally.
